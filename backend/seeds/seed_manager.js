...
async function upsertManager() {
  try {
    const username = 'ucsmanager@gmail.com';
    const plainPassword = 'manager123';

    // Ensure manager role exists
    const roleRes = await db.query('SELECT id FROM roles WHERE role_name = $1', ['manager']);
    let roleId;
    if (roleRes.rows.length === 0) {
      const inserted = await db.query('INSERT INTO roles (role_name) VALUES ($1) RETURNING id', ['manager']);
    ...
    console.log('Manager user ensured:', username);
    process.exit(0);
  } catch (err) {
    console.error('Seed manager failed:', err);
    process.exit(1);
  }
}
